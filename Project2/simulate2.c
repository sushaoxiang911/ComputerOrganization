...
run(stateType state)
{
	...
	int success;
	fetch1:
		printState(&state, "fetch1");
		bus=state.pc;
		state.memoryAddress=bus;
		
	fetch2:
		printState(&state,"fetch2");
		success=memoryAccess(&state,1);
		if(success==1)
		{
			goto decode1;
		}
	...
	sw3:
		...
		goto sw4;	
	sw4:
		printState(&state,"sw5");
		success=memoryAccess(&state,0);
		if(success==1)
		{
			goto fetch1;
		}
